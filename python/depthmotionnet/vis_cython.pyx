...
@cython.boundscheck(False)
cdef _compute_point_cloud_from_depthmap_scaled(
    np.ndarray[np.float32_t, ndim=2] depth,
    np.ndarray[np.float32_t, ndim=3] normals,
    np.ndarray[np.uint8_t, ndim=3] colors,
    np.ndarray[np.float32_t, ndim=2] K,
    ...
    cdef np.ndarray[np.float32_t, ndim=2] points_arr = np.empty((valid_count,3), dtype=np.float32)
    cdef float [:,:] points = points_arr
    cdef np.ndarray[np.float32_t,ndim=2] coordinates_attr_arr = np.empty((valid_count,2), dtype=np.float32)
    cdef float [:,:] coordinates_attr = coordinates_attr_arr
    cdef np.ndarray[np.float32_t,ndim=2] normals_attr_arr = np.empty((valid_count,3), dtype=np.float32)
    cdef float [:,:] normals_attr = normals_attr_arr
    cdef np.ndarray[np.uint8_t,ndim=2] colors_attr_arr = np.empty((valid_count,3), dtype=np.uint8)
    cdef unsigned char [:,:] colors_attr = colors_attr_arr
        ...
        for x in range(width):
            ...
            if isfinite(d) and d > 0:
                ...
                if d < 18*scale:
                    ...
                    points[index,1] = X[1]
                    points[index,2] = X[2]
                    ###### also keep a record of the pixel coordinates of valid 3D points
                    coordinates_attr[index,0] = x
                    coordinates_attr[index,1] = y

                    index += 1

    result = {'points':points_arr}
    result['coordinates'] = coordinates_attr_arr
...
def compute_point_cloud_from_depthmap_scaled( depth, K, R, t, normals=None, colors=None, scale=1 ):
    ...
    return _compute_point_cloud_from_depthmap_scaled(_depth, _normals, colors_arr, K.astype(np.float32), R.astype(np.float32), t.astype(np.float32), scale)


###########################################################################
@cython.boundscheck(False)
cdef _generate_point_cloud_from_depthmap_byPixels_scaled(
    np.ndarray[np.float32_t, ndim=2] depth,
    np.ndarray[np.float32_t, ndim=3] normals,
    np.ndarray[np.uint8_t, ndim=3] colors,
    np.ndarray[np.float32_t, ndim=2] K,
    ...
    for y in range(height):
        for x in range(width):
            ...
            if isfinite(d) and d > 0:
                ...
                points[index,0] = np.nan
                points[index,1] = np.nan
                points[index,2] = np.nan
    ...
    ###### calculate point cloud normals by PCA and patch 7x7 method ######
        index = 0
        for y in range(height):
            for x in range(width):
                d = depth[y,x]
...
def generate_point_cloud_from_depthmap_byPixels_scaled( depth, K, R, t, normals=None, colors=None, scale=1 ):
    ...
    return _generate_point_cloud_from_depthmap_byPixels_scaled(_depth, _normals, colors_arr, K.astype(np.float32), R.astype(np.float32), t.astype(np.float32), scale)
