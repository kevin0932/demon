...
def read_depth_debug(fileurl):
    ...
    """
    response = requests.get(fileurl)
    depth_pil = Image.open(BytesIO(response.content))
    ...
    # plt.imshow(depth_float, cmap=plt.get_cmap('gray'))
    # plt.show()
    return depth_float
...
def compute_sharpness(sun3d_data_path, seq_name):
    ...
    print(sorted(os.listdir(os.path.join(seq_path,'image'))))
...
def compute_sharpness_debug(sun3d_data_path, seq_name, subsamplingRate=50):
    ...
    seq_path = os.path.join(sun3d_data_path,seq_name)
    # # print("seq_path = ", seq_path)
    # for file in listFD(os.path.join(seq_path,'image'), 'jpg'):
    #     print(file)
    ...
    for img_file in image_files:
        # img = read_image(os.path.join(seq_path,'image',img_file))
        print("computing sharpness of ", img_file)
        img = read_image_debug(img_file)
        sharpness.append(measure_sharpness(img))
