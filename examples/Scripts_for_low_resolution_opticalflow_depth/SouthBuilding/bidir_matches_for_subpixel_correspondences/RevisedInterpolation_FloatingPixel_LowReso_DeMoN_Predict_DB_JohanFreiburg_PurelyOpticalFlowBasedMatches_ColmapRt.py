...
def add_multiview_matches_withRt_OpticalFlow(connection, cursor, image_pair12, image_pair23, image_id1, image_id2, image_id3, image_name1, image_name2, image_name3,
                flow12, flow21, flow23, flow32, max_reproj_error, R12_vec, t12_vec, R23_vec, t23_vec, max_photometric_error, img1PIL, img2PIL, img3PIL, real_depth_map1, real_depth_map2, real_depth_map3):
    matches12, coords121, coords122 = flow_to_matches_float32Pixels_withDepthFiltering(flow12, real_depth_map1)
    matches21, coords211, coords212 = flow_to_matches_float32Pixels_withDepthFiltering(flow21, real_depth_map2)
    # matches12, coords121, coords122 = flow_to_matches_withDepthFiltering(flow12, real_depth_map1)
    # matches21, coords211, coords212 = flow_to_matches_withDepthFiltering(flow21, real_depth_map2)
    ...
    # for i1 in range(coords_12_2.shape[0]):
    # for i2 in range(coords_23_2.shape[0]):
    matches_pair12 = []
    ...
    print("check if a triplet track exists: ", matches_pair12.size, ", ", matches_pair23.size)
    ...
    if vals:
        ...
        matches_pair12 = np.concatenate((prev_data, matches_pair12), axis=0)
        float32_coords_pair12_1 = np.concatenate((prev_coords_12_1, float32_coords_pair12_1), axis=0)
        float32_coords_pair12_2 = np.concatenate((prev_coords_12_2, float32_coords_pair12_2), axis=0)
        # print("DEBUG after: final_float32_coords_pair12_1.shape = ", final_float32_coords_pair12_1.shape)
        cursor.execute('''UPDATE inlier_matches SET rows = ? WHERE pair_names = ?''', (matches_pair12.shape[0], image_pair12))
        cursor.execute('''UPDATE inlier_matches SET data = ? WHERE pair_names = ?''', (matches_pair12.tostring(), image_pair12))
        cursor.execute('''UPDATE inlier_matches SET coords_12_1 = ? WHERE pair_names = ?''', (float32_coords_pair12_1.tostring(), image_pair12))
        cursor.execute('''UPDATE inlier_matches SET coords_12_2 = ? WHERE pair_names = ?''', (float32_coords_pair12_2.tostring(), image_pair12))
        connection.commit()
    else:
        cursor.execute("INSERT INTO inlier_matches(pair_names, rows, cols, data, "
                       "config, image_id1, image_id2, rotation, translation, image_name1, image_name2, coords_12_1, coords_12_2) VALUES(?, ?, ?, ?, 3, ?, ?, ?, ?, ?, ?, ?, ?);",
                       (image_pair12, np.array(matches_pair12, dtype=np.uint32).shape[0], np.array(matches_pair12, dtype=np.uint32).shape[1],
                        memoryview(np.array(matches_pair12, dtype=np.uint32)), image_id1, image_id2, memoryview(R12_vec), memoryview(t12_vec), image_name1, image_name2,
                        memoryview(np.array(float32_coords_pair12_1, dtype=np.float32)), memoryview(np.array(float32_coords_pair12_2, dtype=np.float32))))

        connection.commit()

    vals = cursor.execute("SELECT * FROM inlier_matches WHERE pair_names=?", (image_pair23, )).fetchone()
    if vals:
        pair_names, rows, cols, data, config, image_id1, image_id2, rotation, translation, image_name2, image_name3, coords_23_2, coords_23_3 = vals
        prev_data = np.fromstring(data, dtype=np.float32).reshape(-1, 2)
        ...
        matches_pair23 = np.concatenate((prev_data, matches_pair23), axis=0)
        float32_coords_pair23_2 = np.concatenate((prev_coords_23_2, float32_coords_pair23_2), axis=0)
        float32_coords_pair23_3 = np.concatenate((prev_coords_23_3, float32_coords_pair23_3), axis=0)
        cursor.execute('''UPDATE inlier_matches SET rows = ? WHERE pair_names = ?''', (matches_pair23.shape[0], image_pair23))
        cursor.execute('''UPDATE inlier_matches SET data = ? WHERE pair_names = ?''', (matches_pair23.tostring(), image_pair23))
        cursor.execute('''UPDATE inlier_matches SET coords_12_1 = ? WHERE pair_names = ?''', (float32_coords_pair23_2.tostring(), image_pair23))
        cursor.execute('''UPDATE inlier_matches SET coords_12_2 = ? WHERE pair_names = ?''', (float32_coords_pair23_2.tostring(), image_pair23))
        connection.commit()
    else:
        cursor.execute("INSERT INTO inlier_matches(pair_names, rows, cols, data, "
                       "config, image_id1, image_id2, rotation, translation, image_name1, image_name2, coords_12_1, coords_12_2) VALUES(?, ?, ?, ?, 3, ?, ?, ?, ?, ?, ?, ?, ?);",
                       (image_pair23, np.array(matches_pair23, dtype=np.uint32).shape[0], np.array(matches_pair23, dtype=np.uint32).shape[1],
                        memoryview(np.array(matches_pair23, dtype=np.uint32)), image_id2, image_id3, memoryview(R23_vec), memoryview(t23_vec), image_name2, image_name3,
                        memoryview(np.array(float32_coords_pair23_2, dtype=np.float32)), memoryview(np.array(float32_coords_pair23_3, dtype=np.float32))))
...
def main():
    ...
    for image_pair12 in data.keys():
        ...
        for image_pair23 in data.keys():
            ...
            add_multiview_matches_withRt_OpticalFlow(connection, cursor, image_pair12, image_pair23, image_indexGT_from_name1, image_indexGT_from_name2, image_indexGT_from_name3, image_name1, image_name2, image_name3, flow12, flow21, flow23, flow32, args.max_reproj_error, R12_angleaxis, t12_Vec_npfloat32, R23_angleaxis, t23_Vec_npfloat32, args.max_photometric_error, img1PIL, img2PIL, img3PIL, scaled_depth_map1, scaled_depth_map2, scaled_depth_map3)
