...
def main():
    ...
    image_pairs = set()
    valid_pair_num = 0
    # output_file_path = os.path.join(args.output_path, 'BothSideSurvivor_OrderEnforced_filter_'+str(int(args.rotation_consistency_error_deg))+'_'+str(int(args.translation_consistency_error_deg))+'_full_quantization_map_OFscale_'+str(OF_scale_factor)+'_err_'+str(int(args.max_pixel_error*1000))+'_survivorRatio_'+str(int(args.survivor_ratio*1000))+'.txt')
    # output_OFfile_path = os.path.join(args.output_path, 'BothSideSurvivor_OrderEnforced_OpticalFlow_'+str(int(args.rotation_consistency_error_deg))+'_'+str(int(args.translation_consistency_error_deg))+'_full_quantization_map_OFscale_'+str(OF_scale_factor)+'_err_'+str(int(args.max_pixel_error*1000))+'_survivorRatio_'+str(int(args.survivor_ratio*1000))+'.txt')
    output_SurvivorScorefile_path = os.path.join(args.output_path, 'survivor_scores.txt')
    # with open(output_OFfile_path, "w") as fOFid:
    if True:
        #with open(output_file_path_clean, "w") as f2id:
        if True:
            with open(output_SurvivorScorefile_path, "w") as fid:
                ...
                for image_pair12 in data.keys():
                    if True:
                        ...
                        survivor_ratio_12 = matches.shape[0] / matches12.shape[0]
                        ...
                        #
                        # print("rotation_error_from_identity*180/math.pi = ", rotation_error_from_identity*180/math.pi, "; while TransAngularErr*180/math.pi = ", TransAngularErr*180/math.pi)

                        fid.write("%s %s %s %s\n" % (image_pair12, (survivor_ratio_12+survivor_ratio_21)/2, survivor_ratio_12, survivor_ratio_21))

                        # if  (survivor_ratio_12>= args.survivor_ratio and survivor_ratio_21>= args.survivor_ratio) and (rotation_error_from_identity*180/math.pi)<=(args.rotation_consistency_error_deg) and (TransAngularErr*180/math.pi)<=(args.translation_consistency_error_deg):
                        #     valid_pair_num += 1
                        #     print("cross-check-survivor-ratio = ", survivor_ratio_12, "while survivor_ratio_21 = ", survivor_ratio_21)
                            ...
                            # fid.write("%s %s" % (image_name1, image_name2))
                            # # guide_mapping_dict = {}
                            ...
                            #         fOFid.write("%s %s %s %s %s %s %s %s\n" % (flow12[0, y, x], flow12[1, y, x], flow21[0, y, x], flow21[1, y, x], flowconf12[0, y, x], flowconf12[1, y, x], flowconf21[0, y, x], flowconf21[1, y, x]))
                            # fOFid.write("\n") # empty line is added for colmap custom_match format
    ...
    # final_output_file_path = os.path.join(args.output_path, 'BothSideSurvivor_OrderEnforced_filter_'+str(int(args.rotation_consistency_error_deg))+'_'+str(int(args.translation_consistency_error_deg))+'_full_quantization_map_OFscale_'+str(OF_scale_factor)+'_err_'+str(int(args.max_pixel_error*1000))+'_survivorRatio_'+str(int(args.survivor_ratio*1000))+'_validPairNum_'+str(int(valid_pair_num))+'.txt')
    # shutil.copy(output_file_path, final_output_file_path)
    # os.remove(output_file_path)
    # # final_output_file_path_clean = os.path.join(args.output_path, 'BothSideSurvivor_OrderEnforced_Clean_filter_'+str(int(args.rotation_consistency_error_deg))+'_'+str(int(args.translation_consistency_error_deg))+'_full_quantization_map_OFscale_'+str(OF_scale_factor)+'_err_'+str(int(args.max_pixel_error*1000))+'_survivorRatio_'+str(int(args.survivor_ratio*1000))+'_validPairNum_'+str(int(valid_pair_num))+'.txt')
    # # shutil.copy(output_file_path_clean, final_output_file_path_clean)
    # # os.remove(output_file_path_clean)
    # final_output_OFfile_path = os.path.join(args.output_path, 'BothSideSurvivor_OrderEnforced_OpticalFlow_'+str(int(args.rotation_consistency_error_deg))+'_'+str(int(args.translation_consistency_error_deg))+'_full_quantization_map_OFscale_'+str(OF_scale_factor)+'_err_'+str(int(args.max_pixel_error*1000))+'_survivorRatio_'+str(int(args.survivor_ratio*1000))+'_validPairNum_'+str(int(valid_pair_num))+'.txt')
