...
def main():
    ...
    for image_pair12 in data.keys():
        ...
        #             R1=TheiaExtrinsics_T[0:3,0:3],
        #             t1=TheiaExtrinsics_T[0:3,3],
        #             scale=transScale)
        #             # rotation=None,
        #             # translation=None)
        ...
        #
        #
        # # tmp_PointCloud['points'] = transform_pointcloud_points(tmp_PointCloud['points'] / translation_scales[image_name1], TheiaExtrinsics_T.T)
        # # tmp_PointCloud['points'] = transform_pointcloud_points(tmp_PointCloud['points'] * translation_scales[image_name1], TheiaExtrinsics_T.T)
        # # tmp_PointCloud['points'] = transform_pointcloud_points(tmp_PointCloud['points'], TheiaExtrinsics_T.T)
        # print("tmp_PointCloud['points'] = ", tmp_PointCloud['points'])
        # tmp_PointCloud['points'] = transform_pc_to_global_coordinate(tmp_PointCloud['points'], TheiaExtrinsics_T.T)
        # print("tmp_PointCloud['points'] = ", tmp_PointCloud['points'])
        ...
        if image_pair12 in image_pairs:
            ...
            for ids,val in TheiaGlobalPosesGT.items():
                if val.name==image_name1:
                    ...
                    # print("cam_polydata = ", cam_polydata)
                    # #setup colors
                    ...
                    # cam_polydata.GetPointData().SetScalars(Colors)
                    # cam_polydata.Modified()
                    # if vtk.VTK_MAJOR_VERSION <= 5:
                    #     cam_polydata.Update()

                    # cam_polydata = create_camera_polydata(val.rotmat,-np.dot(val.rotmat, val.tvec), False)
                    appendFilter.AddInputData(cam_polydata)
                    # appendFilterModel.AddInputData(cam_polydata)
                    # plywriterCam.SetInputData(cam_polydata)
    appendFilter.Update()
