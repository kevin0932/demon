...
def main():
    ...
    image_pairs = set()
    valid_pair_num = 0
    output_file_path = os.path.join(args.output_path, 'OrderEnforced_filter_'+str(int(args.rotation_consistency_error_deg))+'_'+str(int(args.translation_consistency_error_deg))+'_full_quantization_map_OFscale_'+str(OF_scale_factor)+'_err_'+str(int(args.max_pixel_error*1000))+'_survivorRatio_'+str(int(args.survivor_ratio*1000))+'.txt')
    output_file_path_clean = os.path.join(args.output_path, 'OrderEnforced_Clean_filter_'+str(int(args.rotation_consistency_error_deg))+'_'+str(int(args.translation_consistency_error_deg))+'_full_quantization_map_OFscale_'+str(OF_scale_factor)+'_err_'+str(int(args.max_pixel_error*1000))+'_survivorRatio_'+str(int(args.survivor_ratio*1000))+'.txt')
    output_OFfile_path = os.path.join(args.output_path, 'OrderEnforced_OpticalFlow_'+str(int(args.rotation_consistency_error_deg))+'_'+str(int(args.translation_consistency_error_deg))+'_full_quantization_map_OFscale_'+str(OF_scale_factor)+'_err_'+str(int(args.max_pixel_error*1000))+'_survivorRatio_'+str(int(args.survivor_ratio*1000))+'.txt')
            ...
            with open(output_file_path, "w") as fid:
                ...
                for image_pair12 in data.keys():
                    if True:
                        ...
                        if matches.shape[0] / matches12.shape[0] >= args.survivor_ratio and (rotation_error_from_identity*180/math.pi)<=(args.rotation_consistency_error_deg) and (TransAngularErr*180/math.pi)<=(args.translation_consistency_error_deg):
                            valid_pair_num += 1
