...
def multiview_check_matches_Interpolatedfloat32Pixel(matches01, coords_01_0, coords_01_1, matches12, coords121, coords122,
                        matches21, coords211, coords212, max_reproj_error, flow12, flow21):
    if matches12.size == 0 or matches21.size == 0:
        return np.zeros((0, 2), dtype=np.uint32)
...
def add_multiview_tripletclosure_matches_withRt_OpticalFlow(connection, cursor, image_pair12, image_pair23, image_pair13, image_id1, image_id2, image_id3, image_name1, image_name2, image_name3,
                flow12, flow21, flow23, flow32, flow13, flow31, max_reproj_error, R12_vec, t12_vec, R23_vec, t23_vec, R13_vec, t13_vec, max_photometric_error, img1PIL, img2PIL, img3PIL, real_depth_map1, real_depth_map2, real_depth_map3):
    matches12, coords121, coords122 = flow_to_matches_float32Pixels_withDepthFiltering(flow12, real_depth_map1)
    matches21, coords211, coords212 = flow_to_matches_float32Pixels_withDepthFiltering(flow21, real_depth_map2)
    # matches12, coords121, coords122 = flow_to_matches_withDepthFiltering(flow12, real_depth_map1)
    # matches21, coords211, coords212 = flow_to_matches_withDepthFiltering(flow21, real_depth_map2)
    ...
    if matches.size == 0:
        return
    print("  => 1---2: Cross-checked", matches.shape[0], "matches")
    ...
    # print("  => photo-checked", matches.shape[0]/2, "matches")
    # if matches.size == 0:
    #     return

    ### trial to connect to the third view
    matches23, coords231, coords232 = flow_to_matches_float32Pixels_withDepthFiltering(flow23, real_depth_map2)
    matches32, coords321, coords322 = flow_to_matches_float32Pixels_withDepthFiltering(flow32, real_depth_map3)
    print("  => Found", matches23.size/2, "<->", matches32.size/2, "matches")
    if  matches23.size/2 <= 0 or matches32.size/2 <= 0:
        return
    matches_2ndPair, coords_23_2, coords_23_3 = cross_check_matches_Interpolatedfloat32Pixel(matches23, coords231, coords232,
                                  matches32, coords321, coords322,
                                  max_reproj_error, flow23, flow32)
    print("matches_2ndPair.shape = ", matches_2ndPair.shape, "; ", "coords_23_2.shape = ", coords_23_2.shape, "coords_23_3.shape = ", coords_23_3.shape)

    if matches_2ndPair.size == 0:
        return
    print("  => 2---3: Cross-checked", matches_2ndPair.shape[0], "matches")

    ### merge the track within 1-2-3 triplet
    matches_pair12 = []
    float32_coords_pair12_1 = []
    float32_coords_pair12_2 = []
    matches_pair23 = []
    float32_coords_pair23_2 = []
    float32_coords_pair23_3 = []

    tmpDistMat = cdist(coords_12_2, coords_23_2)
    minCandidate2Idxfor1 = np.argmin(tmpDistMat,axis=1)
    for idx in range(minCandidate2Idxfor1.shape[0]):
        if tmpDistMat[idx, minCandidate2Idxfor1[idx]] < max_reproj_error:
            coords_1 = coords_12_1[idx]
            coords_2 = (coords_12_2[idx]+coords_23_2[minCandidate2Idxfor1[idx]])/2
            coords_3 = coords_23_3[minCandidate2Idxfor1[idx]]
            match12_12 = matches[idx]
            match23_23 = matches_2ndPair[minCandidate2Idxfor1[idx]]

            matches_pair12.append((match12_12[0], match12_12[1]))
            float32_coords_pair12_1.append( coords_1 )
            float32_coords_pair12_2.append( coords_2 )
            matches_pair23.append((match23_23[0], match23_23[1]))
            float32_coords_pair23_2.append( coords_2 )
            float32_coords_pair23_3.append( coords_3 )

    # return np.array(matches, dtype=np.uint32), np.array(float32_coords_1, dtype=np.float32), np.array(float32_coords_2, dtype=np.float32)
    matches_pair12 = np.array(matches_pair12, dtype=np.uint32)
    matches_pair23 = np.array(matches_pair23, dtype=np.uint32)
    float32_coords_pair12_1 = np.array(float32_coords_pair12_1, dtype=np.float32)
    float32_coords_pair12_2 = np.array(float32_coords_pair12_2, dtype=np.float32)
    float32_coords_pair23_2 = np.array(float32_coords_pair23_2, dtype=np.float32)
    float32_coords_pair23_3 = np.array(float32_coords_pair23_3, dtype=np.float32)

    print("check if a triplet track exists: ", matches_pair12.size/2, ", ", matches_pair23.size/2)
    if matches_pair12.size == 0 or matches_pair23.size ==0:
        return

    ############### trial to close the triplet constraint
    matches13, coords131, coords132 = flow_to_matches_float32Pixels_withDepthFiltering(flow13, real_depth_map1)
    matches31, coords311, coords312 = flow_to_matches_float32Pixels_withDepthFiltering(flow31, real_depth_map3)
    print("  => Found", matches13.size/2, "<->", matches31.size/2, "matches")
    if  matches13.size/2 <= 0 or matches31.size/2 <= 0:
        return
    matches_3rdPair, coords_13_1, coords_13_3 = cross_check_matches_Interpolatedfloat32Pixel(matches13, coords131, coords132,
                                  matches31, coords311, coords312,
                                  max_reproj_error, flow13, flow31)
    print("matches_3rdPair.shape = ", matches_3rdPair.shape, "; ", "coords_13_1.shape = ", coords_13_1.shape, "coords_13_3.shape = ", coords_13_3.shape)

    if matches_3rdPair.size == 0:
        return
    print("  => 1---3: Cross-checked", matches_3rdPair.shape[0], "matches")

    ############ merge the track with the 3rd pair within 1-2-3 triplet
    final_matches_pair12 = []
    final_float32_coords_pair12_1 = []
    final_float32_coords_pair12_2 = []
    final_matches_pair23 = []
    final_float32_coords_pair23_2 = []
    final_float32_coords_pair23_3 = []
    final_matches_pair13 = []
    final_float32_coords_pair13_1 = []
    final_float32_coords_pair13_3 = []

    tmpDistMat = cdist(float32_coords_pair12_1, coords_13_1)
    minCandidate3Idxfor1 = np.argmin(tmpDistMat,axis=1)
    for idx in range(minCandidate3Idxfor1.shape[0]):
        if tmpDistMat[idx, minCandidate3Idxfor1[idx]] < max_reproj_error:
            coords_1 = (float32_coords_pair12_1[idx]+coords_13_1[minCandidate3Idxfor1[idx]])/2
            coords_2 = float32_coords_pair12_2[idx]
            coords_3 = (float32_coords_pair23_3[idx]+coords_13_3[minCandidate3Idxfor1[idx]])/2
            match12_12 = matches_pair12[idx]
            match23_23 = matches_pair23[idx]
            match13_13 = matches_3rdPair[minCandidate3Idxfor1[idx]]

            final_matches_pair12.append((match12_12[0], match12_12[1]))
            final_float32_coords_pair12_1.append( coords_1 )
            final_float32_coords_pair12_2.append( coords_2 )
            final_matches_pair23.append((match23_23[0], match23_23[1]))
            final_float32_coords_pair23_2.append( coords_2 )
            final_float32_coords_pair23_3.append( coords_3 )
            final_matches_pair13.append((match13_13[0], match13_13[1]))
            final_float32_coords_pair13_1.append( coords_1 )
            final_float32_coords_pair13_3.append( coords_3 )

    # return np.array(matches, dtype=np.uint32), np.array(float32_coords_1, dtype=np.float32), np.array(float32_coords_2, dtype=np.float32)
    final_matches_pair12 = np.array(final_matches_pair12, dtype=np.uint32)
    final_matches_pair23 = np.array(final_matches_pair23, dtype=np.uint32)
    final_matches_pair13 = np.array(final_matches_pair13, dtype=np.uint32)
    final_float32_coords_pair12_1 = np.array(final_float32_coords_pair12_1, dtype=np.float32)
    final_float32_coords_pair12_2 = np.array(final_float32_coords_pair12_2, dtype=np.float32)
    final_float32_coords_pair23_2 = np.array(final_float32_coords_pair23_2, dtype=np.float32)
    final_float32_coords_pair23_3 = np.array(final_float32_coords_pair23_3, dtype=np.float32)
    final_float32_coords_pair13_1 = np.array(final_float32_coords_pair13_1, dtype=np.float32)
    final_float32_coords_pair13_3 = np.array(final_float32_coords_pair13_3, dtype=np.float32)

    print("check if closed triplet track exists: ", final_matches_pair12.size/2, ", ", final_matches_pair23.size/2, ", ", final_matches_pair13.size/2)
    if final_matches_pair12.size == 0 or final_matches_pair23.size == 0 or final_matches_pair13.size == 0:
        return

    vals = cursor.execute("SELECT * FROM inlier_matches WHERE pair_names=?", (image_pair12, )).fetchone()
    if vals:
        # print("DEBUG before: final_float32_coords_pair12_1.shape = ", final_float32_coords_pair12_1.shape)
        pair_names, rows, cols, data, config, image_id1, image_id2, rotation, translation, image_name1, image_name2, coords_12_1, coords_12_2 = vals
        # print("DEBUG pair_names = ", pair_names)
        prev_data = np.fromstring(data, dtype=np.float32).reshape(-1, 2)
        prev_coords_12_1 = np.fromstring(coords_12_1, dtype=np.float32).reshape(-1, 2)
        prev_coords_12_2 = np.fromstring(coords_12_2, dtype=np.float32).reshape(-1, 2)
        # print("DEBUG before: prev_coords_12_1.shape = ", prev_coords_12_1.shape)
        final_matches_pair12 = np.concatenate((prev_data, final_matches_pair12), axis=0)
        final_float32_coords_pair12_1 = np.concatenate((prev_coords_12_1, final_float32_coords_pair12_1), axis=0)
        final_float32_coords_pair12_2 = np.concatenate((prev_coords_12_2, final_float32_coords_pair12_2), axis=0)
        # print("DEBUG after: final_float32_coords_pair12_1.shape = ", final_float32_coords_pair12_1.shape)
        cursor.execute('''UPDATE inlier_matches SET rows = ? WHERE pair_names = ?''', (final_matches_pair12.shape[0], image_pair12))
        cursor.execute('''UPDATE inlier_matches SET data = ? WHERE pair_names = ?''', (final_matches_pair12.tostring(), image_pair12))
        cursor.execute('''UPDATE inlier_matches SET coords_12_1 = ? WHERE pair_names = ?''', (final_float32_coords_pair12_1.tostring(), image_pair12))
        cursor.execute('''UPDATE inlier_matches SET coords_12_2 = ? WHERE pair_names = ?''', (final_float32_coords_pair12_2.tostring(), image_pair12))
        connection.commit()
    else:
        cursor.execute("INSERT INTO inlier_matches(pair_names, rows, cols, data, "
                       "config, image_id1, image_id2, rotation, translation, image_name1, image_name2, coords_12_1, coords_12_2) VALUES(?, ?, ?, ?, 3, ?, ?, ?, ?, ?, ?, ?, ?);",
                       (image_pair12, np.array(final_matches_pair12, dtype=np.uint32).shape[0], np.array(final_matches_pair12, dtype=np.uint32).shape[1],
                        memoryview(np.array(final_matches_pair12, dtype=np.uint32)), image_id1, image_id2, memoryview(R12_vec), memoryview(t12_vec), image_name1, image_name2,
                        memoryview(np.array(final_float32_coords_pair12_1, dtype=np.float32)), memoryview(np.array(final_float32_coords_pair12_2, dtype=np.float32))))

        connection.commit()

    vals = cursor.execute("SELECT * FROM inlier_matches WHERE pair_names=?", (image_pair23, )).fetchone()
    if vals:
        pair_names, rows, cols, data, config, image_id2, image_id3, rotation, translation, image_name2, image_name3, coords_23_2, coords_23_3 = vals
        prev_data = np.fromstring(data, dtype=np.float32).reshape(-1, 2)
        prev_coords_23_2 = np.fromstring(coords_23_2, dtype=np.float32).reshape(-1, 2)
        prev_coords_23_3 = np.fromstring(coords_23_3, dtype=np.float32).reshape(-1, 2)
        final_matches_pair23 = np.concatenate((prev_data, final_matches_pair23), axis=0)
        final_float32_coords_pair23_2 = np.concatenate((prev_coords_23_2, final_float32_coords_pair23_2), axis=0)
        final_float32_coords_pair23_3 = np.concatenate((prev_coords_23_3, final_float32_coords_pair23_3), axis=0)
        cursor.execute('''UPDATE inlier_matches SET rows = ? WHERE pair_names = ?''', (final_matches_pair23.shape[0], image_pair23))
        cursor.execute('''UPDATE inlier_matches SET data = ? WHERE pair_names = ?''', (final_matches_pair23.tostring(), image_pair23))
        cursor.execute('''UPDATE inlier_matches SET coords_12_1 = ? WHERE pair_names = ?''', (final_float32_coords_pair23_2.tostring(), image_pair23))
        cursor.execute('''UPDATE inlier_matches SET coords_12_2 = ? WHERE pair_names = ?''', (final_float32_coords_pair23_3.tostring(), image_pair23))
        connection.commit()
    else:
        cursor.execute("INSERT INTO inlier_matches(pair_names, rows, cols, data, "
                   "config, image_id1, image_id2, rotation, translation, image_name1, image_name2, coords_12_1, coords_12_2) VALUES(?, ?, ?, ?, 3, ?, ?, ?, ?, ?, ?, ?, ?);",
                   (image_pair23, np.array(final_matches_pair23, dtype=np.uint32).shape[0], np.array(final_matches_pair23, dtype=np.uint32).shape[1],
                    memoryview(np.array(final_matches_pair23, dtype=np.uint32)), image_id2, image_id3, memoryview(R23_vec), memoryview(t23_vec), image_name2, image_name3,
                    memoryview(np.array(final_float32_coords_pair23_2, dtype=np.float32)), memoryview(np.array(final_float32_coords_pair23_3, dtype=np.float32))))

        connection.commit()

    vals = cursor.execute("SELECT * FROM inlier_matches WHERE pair_names=?", (image_pair13, )).fetchone()
    if vals:
        pair_names, rows, cols, data, config, image_id1, image_id3, rotation, translation, image_name1, image_name3, coords_13_1, coords_13_3 = vals
        prev_data = np.fromstring(data, dtype=np.float32).reshape(-1, 2)
        prev_coords_13_1 = np.fromstring(coords_13_1, dtype=np.float32).reshape(-1, 2)
        prev_coords_13_3 = np.fromstring(coords_13_3, dtype=np.float32).reshape(-1, 2)
        final_matches_pair13 = np.concatenate((prev_data, final_matches_pair13), axis=0)
        final_float32_coords_pair13_1 = np.concatenate((prev_coords_13_1, final_float32_coords_pair13_1), axis=0)
        final_float32_coords_pair13_3 = np.concatenate((prev_coords_13_3, final_float32_coords_pair13_3), axis=0)
        cursor.execute('''UPDATE inlier_matches SET rows = ? WHERE pair_names = ?''', (final_matches_pair13.shape[0], image_pair13))
        cursor.execute('''UPDATE inlier_matches SET data = ? WHERE pair_names = ?''', (final_matches_pair13.tostring(), image_pair13))
        cursor.execute('''UPDATE inlier_matches SET coords_12_1 = ? WHERE pair_names = ?''', (final_float32_coords_pair13_1.tostring(), image_pair13))
        cursor.execute('''UPDATE inlier_matches SET coords_12_2 = ? WHERE pair_names = ?''', (final_float32_coords_pair13_3.tostring(), image_pair13))
        connection.commit()
    else:
        cursor.execute("INSERT INTO inlier_matches(pair_names, rows, cols, data, "
                   "config, image_id1, image_id2, rotation, translation, image_name1, image_name2, coords_12_1, coords_12_2) VALUES(?, ?, ?, ?, 3, ?, ?, ?, ?, ?, ?, ?, ?);",
                   (image_pair23, np.array(final_matches_pair13, dtype=np.uint32).shape[0], np.array(final_matches_pair13, dtype=np.uint32).shape[1],
                    memoryview(np.array(final_matches_pair13, dtype=np.uint32)), image_id1, image_id3, memoryview(R13_vec), memoryview(t13_vec), image_name1, image_name3,
                    memoryview(np.array(final_float32_coords_pair13_1, dtype=np.float32)), memoryview(np.array(final_float32_coords_pair13_3, dtype=np.float32))))
...
def main():
    ...
    ### Try to fix the inconsistent image id issue when importing to matchfile.cereal
    ...
    for image_pair12 in data.keys():
        image_name_12_1, image_name_12_2 = image_pair12.split("---")
        image_pair_12_21 = "{}---{}".format(image_name_12_2, image_name_12_1)
        if image_pair_12_21 not in data:
            continue

        if image_pair_12_21 not in good_pairs or image_pair12 not in good_pairs :
            print("skip the image pair because it is not a good pair by visual inspection!")
            continue

        for image_pair23 in data.keys():
            print("Processing", image_pair12, " and ", image_pair23)
            image_name_23_2, image_name_23_3 = image_pair23.split("---")
            image_pair_23_32 = "{}---{}".format(image_name_23_3, image_name_23_2)
            if image_pair_23_32 not in data:
                continue

            if image_name_12_2!=image_name_23_2:
                print('############################ skipping the same image2 triplet')
                continue

            image_triplet_123 = "{}---{}---{}".format(image_name_12_1, image_name_12_2, image_name_23_3)
            if image_triplet_123 in image_triplets:
                continue
            image_triplets.add(image_triplet_123)

            image_name1 = image_name_12_1
            image_name2 = image_name_12_2
            image_name3 = image_name_23_3

            image_pair13 = "{}---{}".format(image_name1, image_name3)
            image_pair_13_31 = "{}---{}".format(image_name3, image_name1)
            if image_pair13 not in data or image_pair_13_31 not in data:
                print("~~~~~~~~~~~~~~~~~ Skip the triplet because of the lack of the 3rd pair in the given DeMoN prediction! ~~~~~~~~~~~~~~~~~")
                continue

            if image_pair_23_32 not in good_pairs or image_pair23 not in good_pairs :
                print("skip the triplet because the 2nd pair is not a good pair by visual inspection!")
                continue

            if image_pair_13_31 not in good_pairs or image_pair13 not in good_pairs :
                print("skip the triplet because the 3rd pair is not a good pair by visual inspection!")
                continue
            ...
            #             connection, cursor, args.focal_length, image_name2,
            #             np.array([48, 64]), args.image_scale, np.zeros(4), np.zeros(3), np.zeros(3))
            # if image_name3 not in images.keys():
            #     images[image_name3] = add_image_withRt(
            #             connection, cursor, args.focal_length, image_name3,
            #             np.array([48, 64]), args.image_scale, np.zeros(4), np.zeros(3), np.zeros(3))
            # if image_name1 not in imagesNoRt.keys():
            #     imagesNoRt[image_name1] = add_image_withRt_colmap(
            #             connectionNoRt, cursorNoRt, args.focal_length, image_name1,
            ...
            #             connectionNoRt, cursorNoRt, args.focal_length, image_name2,
            #             np.array([48, 64]), args.image_scale, np.zeros(4), np.zeros(3))
            # if image_name3 not in imagesNoRt.keys():
            #     imagesNoRt[image_name3] = add_image_withRt_colmap(
            #             connectionNoRt, cursorNoRt, args.focal_length, image_name3,
            #             np.array([48, 64]), args.image_scale, np.zeros(4), np.zeros(3))

            image_indexGT_from_name1 = images[image_name1]
            image_indexGT_from_name2 = images[image_name2]
            image_indexGT_from_name3 = images[image_name3]
            ...
            image_pair23_rotmat = np.dot(view3Colmap.R, view2Colmap.R.T)
            image_pair32_rotmat = np.dot(view2Colmap.R, view3Colmap.R.T)
            image_pair13_rotmat = np.dot(view3Colmap.R, view1Colmap.R.T)
            image_pair31_rotmat = np.dot(view1Colmap.R, view3Colmap.R.T)
            ...
            image_pair23_transVec = -np.dot(np.dot(view3Colmap.R, view2Colmap.R.T), np.dot(view2Colmap.R, (-np.dot(view3Colmap.R.T, view3Colmap.t) + np.dot(view2Colmap.R.T, view2Colmap.t))) )
            image_pair32_transVec = -np.dot(np.dot(view2Colmap.R, view3Colmap.R.T), np.dot(view3Colmap.R, (-np.dot(view2Colmap.R.T, view2Colmap.t) + np.dot(view3Colmap.R.T, view3Colmap.t))) )
            image_pair13_transVec = -np.dot(np.dot(view3Colmap.R, view1Colmap.R.T), np.dot(view1Colmap.R, (-np.dot(view3Colmap.R.T, view3Colmap.t) + np.dot(view1Colmap.R.T, view1Colmap.t))) )
            image_pair31_transVec = -np.dot(np.dot(view1Colmap.R, view3Colmap.R.T), np.dot(view3Colmap.R, (-np.dot(view1Colmap.R.T, view1Colmap.t) + np.dot(view3Colmap.R.T, view3Colmap.t))) )


            scaled_depth_map1 = view1Colmap.depth
            scaled_depth_map2 = view2Colmap.depth
            scaled_depth_map3 = view3Colmap.depth
            flow12 = data[image_pair12]["flow"]
            # flow12 = np.transpose(flow12, [2, 0, 1])
            print(flow12.shape)
            flow21 = data[image_pair_12_21]["flow"]
            # flow21 = np.transpose(flow21, [2, 0, 1])
            print(flow21.shape)
            flow23 = data[image_pair23]["flow"]
            # flow23 = np.transpose(flow23, [2, 0, 1])
            print(flow23.shape)
            flow32 = data[image_pair_23_32]["flow"]
            # flow32 = np.transpose(flow32, [2, 0, 1])
            print(flow32.shape)
            flow13 = data[image_pair13]["flow"]
            # flow13 = np.transpose(flow13, [2, 0, 1])
            print(flow13.shape)
            flow31 = data[image_pair_13_31]["flow"]
            # flow31 = np.transpose(flow31, [2, 0, 1])
            print(flow31.shape)

            eulerAnlges12 = mat2euler(image_pair12_rotmat)
            recov_angle_axis_result12 = euler2angle_axis(eulerAnlges12[0], eulerAnlges12[1], eulerAnlges12[2])
            R12_angleaxis = recov_angle_axis_result12[0]*(recov_angle_axis_result12[1])
            R12_angleaxis = np.array(R12_angleaxis, dtype=np.float32)
            t12_Vec_npfloat32 = np.array(-np.dot(image_pair12_rotmat.T,image_pair12_transVec), dtype=np.float32)

            ### add the pair 21 as well
            eulerAnlges21 = mat2euler(image_pair21_rotmat)
            recov_angle_axis_result21 = euler2angle_axis(eulerAnlges21[0], eulerAnlges21[1], eulerAnlges21[2])
            R21_angleaxis = recov_angle_axis_result21[0]*(recov_angle_axis_result21[1])
            R21_angleaxis = np.array(R21_angleaxis, dtype=np.float32)
            t21_Vec_npfloat32 = np.array(-np.dot(image_pair21_rotmat.T,image_pair21_transVec), dtype=np.float32)

            #####################################################################################################
            eulerAnlges23 = mat2euler(image_pair23_rotmat)
            recov_angle_axis_result23 = euler2angle_axis(eulerAnlges23[0], eulerAnlges23[1], eulerAnlges23[2])
            R23_angleaxis = recov_angle_axis_result23[0]*(recov_angle_axis_result23[1])
            R23_angleaxis = np.array(R23_angleaxis, dtype=np.float32)
            t23_Vec_npfloat32 = np.array(-np.dot(image_pair23_rotmat.T,image_pair23_transVec), dtype=np.float32)

            ### add the pair 21 as well
            eulerAnlges32 = mat2euler(image_pair32_rotmat)
            recov_angle_axis_result32 = euler2angle_axis(eulerAnlges32[0], eulerAnlges32[1], eulerAnlges32[2])
            R32_angleaxis = recov_angle_axis_result32[0]*(recov_angle_axis_result32[1])
            R32_angleaxis = np.array(R32_angleaxis, dtype=np.float32)
            t32_Vec_npfloat32 = np.array(-np.dot(image_pair32_rotmat.T,image_pair32_transVec), dtype=np.float32)

            #####################################################################################################
            eulerAnlges13 = mat2euler(image_pair13_rotmat)
            recov_angle_axis_result13 = euler2angle_axis(eulerAnlges13[0], eulerAnlges13[1], eulerAnlges13[2])
            R13_angleaxis = recov_angle_axis_result13[0]*(recov_angle_axis_result13[1])
            R13_angleaxis = np.array(R13_angleaxis, dtype=np.float32)
            t13_Vec_npfloat32 = np.array(-np.dot(image_pair13_rotmat.T,image_pair13_transVec), dtype=np.float32)

            ### add the pair 31 as well
            eulerAnlges31 = mat2euler(image_pair31_rotmat)
            recov_angle_axis_result31 = euler2angle_axis(eulerAnlges31[0], eulerAnlges31[1], eulerAnlges31[2])
            R31_angleaxis = recov_angle_axis_result31[0]*(recov_angle_axis_result31[1])
            R31_angleaxis = np.array(R31_angleaxis, dtype=np.float32)
            t31_Vec_npfloat32 = np.array(-np.dot(image_pair31_rotmat.T,image_pair31_transVec), dtype=np.float32)


            add_multiview_tripletclosure_matches_withRt_OpticalFlow(connection, cursor, image_pair12, image_pair23, image_pair13, image_indexGT_from_name1, image_indexGT_from_name2, image_indexGT_from_name3, image_name1, image_name2, image_name3, flow12, flow21, flow23, flow32, flow13, flow31, args.max_reproj_error, R12_angleaxis, t12_Vec_npfloat32, R23_angleaxis, t23_Vec_npfloat32, R13_angleaxis, t13_Vec_npfloat32, args.max_photometric_error, img1PIL, img2PIL, img3PIL, scaled_depth_map1, scaled_depth_map2, scaled_depth_map3)
            ...
            relativePoses_outputGTfile.write("%s %s %s %s %s %s %s %f %f %f %f %f %f\n" % (image_pair12, image_indexGT_from_name1, images[image_name1], image_name1, image_indexGT_from_name2, images[image_name2], image_name2, t12_Vec_npfloat32[0], t12_Vec_npfloat32[1], t12_Vec_npfloat32[2], R12_angleaxis[0], R12_angleaxis[1], R12_angleaxis[2]))
