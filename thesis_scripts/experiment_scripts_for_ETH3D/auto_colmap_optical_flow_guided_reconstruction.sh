...
do
  ...
  find $exhaustive_prediction_dir -name "BothSideSurvivor_OrderEnforced_filter_360_360_full_quantization_map_$image_pair_retrieval_params*.txt" -print0 | while read -d $'\0' match_list_file
  do
    echo $match_list_file

    find $exhaustive_prediction_dir -name "BothSideSurvivor_OrderEnforced_OpticalFlow_360_360_full_quantization_map_$image_pair_retrieval_params*.txt" -print0 | while read -d $'\0' optical_flow_file
    do
